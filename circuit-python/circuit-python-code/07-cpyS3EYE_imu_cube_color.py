"""
07-cpyS3EYE_imu_cube_color.py
3D Wireframe Cube with Colored Axes
X-axis edges: Red
Y-axis edges: Green
Z-axis edges: Blue
...
# We need 4 colors now: Black + Red + Green + Blue
bitmap = displayio.Bitmap(display.width, display.height, 4)
palette = displayio.Palette(4)
palette[0] = 0x000000  # Black background
palette[1] = 0xFF0000  # Red (X-axis)
palette[2] = 0x00FF00  # Green (Y-axis)
palette[3] = 0x0088FF  # Light Blue (Z-axis)
...
# (Point A, Point B, Color Index)
edges = [
    # X-axis edges (Red)
    (0, 1, 1), (3, 2, 1), (4, 5, 1), (7, 6, 1),
    # Y-axis edges (Green)
    (0, 3, 2), (1, 2, 2), (4, 7, 2), (5, 6, 2),
    # Z-axis edges (Blue)
...
while True:
    ...
    # Draw wireframe with colors
    for a, b, color in edges:
        bitmaptools.draw_line(bitmap, px[a], py[a], px[b], py[b], color)
